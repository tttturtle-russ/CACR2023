...
int main() {
    ...
    char *receiver = (char *)malloc(128 * sizeof (char));
    ...
    if (mosquitto_connect(mosq,HOST,PORT,0) != MOSQ_ERR_SUCCESS){
        printf("mosquitto connect failed\n");
        return 1;
    }
    mosquitto_loop_start(mosq);
    mosquitto_publish(mosq,nullptr,"test",strlen("test"),"test",0,false);
    ...
    while (true) {
        ...
        switch (choice) {
            case 1:
                ...
                scanf("%s",receiver);
                generate_p2p(msg,sender,receiver);
                break;
            case 2:
                std::cout << "input message" << std::endl;
                scanf("%s",msg);
