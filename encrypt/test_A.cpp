...
int read_pem(const std::string& uuid,SM2_KEY *sm2_key,uint8_t sm4_key_arr[16],uint8_t sm4_key_iv[16],uint8_t sm3_hmac_key_arr[16]) {
    ...
    auto pems = (client).database("mqGate").collection("pems");
    if (!pems){
        return 1;
    }
...
int main() {
    ...
    //connOpts.set_user_name("001XZEKNYB6r7NvZ");
    //connOpts.set_password("russ");
    std::string msg;
    std::string sender("a87fd0eec32a4a60a07b0103247b6a45");
    std::cout << "emulate sender:" << sender << std::endl;
    std::string receiver;
    ...
    mqtt::message_ptr p2pmsg = nullptr;
        ...
        client.subscribe("/p2p/a87fd0eec32a4a60a07b0103247b6a45");
        while (true) {
            usage();
            std::cin >> choice;
            std::cin.get();
            switch (choice) {
                case 1:
                    ...
                    std::cout << "send p2p message" << std::endl;
                    break;
                case 2:
                    std::cout << "input message" << std::endl;
                    std::getline(std::cin,msg);
                    ...
                    std::cout << "send public message" << std::endl;
                    break;
                case 3:
                    std::cout << "input message" << std::endl;
                    std::getline(std::cin,msg);
                    ...
                    client.publish(pubmsg);
                    std::cout << "send checkpoint message" << std::endl;
                    break;
                case 4:
                    std::cout << "input message" << std::endl;
                    std::getline(std::cin,msg);
                    ...
                    client.publish(pubmsg);
                    std::cout << "send accident message" << std::endl;
                    break;
                default:
                    usage();
                    break;
